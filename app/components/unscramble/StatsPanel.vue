  ...
  <div class="w-full">
    <div class="flex justify-center gap-4 text-center mb-4">
      <div>
        ...
        <p class="text-xs text-slate-500">Played</p>
      </div>
      <div>
        <p class="text-2xl font-bold">
          {{ s.gamesPlayed ? Math.round((s.gamesWon / s.gamesPlayed) * 100) : 0 }}%
        </p>
        <p class="text-xs text-slate-500">Win %</p>
      </div>
      <div>
    ...
    <!-- Guess distribution -->
    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-2 text-center">
      Guess Distribution
    </p>
    <div class="flex flex-col gap-1">
      ...
      >
        <span class="w-3 text-slate-500 font-semibold">{{ n }}</span>
        <div class="flex-1 bg-slate-100 rounded overflow-hidden">
          <div
            class="h-5 rounded flex items-center justify-end pr-1 text-xs font-bold text-white transition-all"
            :class="s.guessDistribution[n] ? 'bg-indigo-500' : 'bg-slate-200'"
